...
const ClapCount = ({ className }) => {
    ...
    return <span ref={setRef}
        data-refkey='clapCountRef' className={classNames}>
        + {count}
    </span>
};
...
const CountTotal = ({ className }) => {
    ...
    return <span ref={setRef}
        data-refkey='countTotalRef' className={classNames}>
        {countTotal}
    </span>
}
...
const ClapIcon = ({ className }) => {
    ...
    const classNames = useMemo(() => [styles.icon, className].join(' ').trim(), [className]);

    return <span>
        <svg id="clap-icon"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="-549 338 100.1 125"
            className={`${classNames} ${isClicked && styles.checked}`}>
